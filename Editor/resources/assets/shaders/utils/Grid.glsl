...
#define DepthRangeNear    0.1
#define DepthRangeFar     100.0
...
vec4 CreateGrid(vec3 gridColor, vec3 fragPos3D, float scale, bool drawAxis) 
{
    ...
    // Z axis
    if(fragPos3D.x > - 1.0 * MinX && fragPos3D.x < 1.0 * MinX)
       Color.rgb = vec3(0.2, 0.2, 0.9);
    // X axis
    if(fragPos3D.z > - 1.0 * MinZ && fragPos3D.z < 1.0 * MinZ)
        Color.rgb = vec3(0.9, 0.2, 0.2);
...
float ComputeLinearDepth(vec3 position) 
{
    ...
    return LinearDepth / DepthRangeFar; 
}
...
void main()
{
    ...
    vec4 Color          = (CreateGrid(GridColor, FragPos3D, 0.2, true) + CreateGrid(GridColor, FragPos3D, 1.0, true)) * float(T > 0);
