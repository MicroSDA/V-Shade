...
shade::VulkanUniformBuffer::VulkanUniformBuffer(const VkDevice& device, VulkanContext::VulkanInstance& instance, Usage usage, std::uint32_t binding, std::uint32_t size, std::uint32_t framesCount, std::size_t resizeThreshold):
	m_Binding(binding),
	m_Size(size),
...
shade::VulkanUniformBuffer::~VulkanUniformBuffer()
{
...
void shade::VulkanUniformBuffer::Invalidate(std::uint32_t binding, std::uint32_t size, std::uint32_t framesCount, std::uint32_t resizeThreshold)
{
...
void shade::VulkanUniformBuffer::SetData(std::uint32_t size, const void* data, std::uint32_t frameIndex, std::uint32_t offset)
{
	if (size)
	{
		// if the usage is GPU
		if (m_Usage == Usage::GPU)
		{
			...
			VkBufferCopy copyRegion{ 0, (frameIndex * m_AlignmentSize) + offset, size };
		...
		{
			// Map the staging device memory to pData to copy the data to it.
			const std::uint32_t newOffset = (frameIndex * m_AlignmentSize) + offset;
...
void shade::VulkanUniformBuffer::Resize(std::uint32_t size)
{
...
std::uint32_t shade::VulkanUniformBuffer::GetBinding()
{
	return m_Binding;
}

const VkBuffer shade::VulkanUniformBuffer::GetBuffer() const
{
	return m_Buffer;
}

std::tuple<VkBuffer, VkDeviceMemory> shade::VulkanUniformBuffer::CreateBuffer(VkDevice device, const VkAllocationCallbacks* pAllocator, const VkBufferCreateInfo* pCreateInfo, const VkMemoryPropertyFlags& properties)
{
	VkBuffer buffer;
...
std::uint32_t shade::VulkanUniformBuffer::GetBufferMinAlignment(std::size_t size)
{
	// Calculate required alignment based on minimum device offset alignment
	size_t minSboAlignment = VulkanContext::GetPhysicalDevice()->GetPhysicalDeviceProperties().limits.minUniformBufferOffsetAlignment;
