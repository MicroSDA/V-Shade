...
namespace shade
{
	...
	public:
		...
		virtual void DrawImageLayerd(SharedPointer<Texture2D>& texture, const ImVec2& size, const ImVec4& borderColor, std::uint32_t layer, bool isGrayScale, const glm::vec2& clamp) override;
	private:
		...
		std::unordered_map<std::size_t, std::pair<VkDescriptorSet, VkImageView>> m_Images;
	};
}