...
void shade::Renderer::EndFrame(std::uint32_t frameIndex)
{
	...
	m_sRenderAPI->m_sSceneRenderData.PointLightCount = 0;
...
void shade::Renderer::BeginScene(SharedPointer<Camera>& camera, const RenderAPI::RenderSettings& renderSettings, std::uint32_t frameIndex)
{
	...
	m_sRenderAPI->m_sRenderSettings.ShadowSettings.GlobalLight = GlobalLight::GetRenderShadowSettings();
	...
	m_sRenderAPI->m_sSubmitedSceneRenderData.PointsLightsBuffer->SetData(POINT_LIGHTS_DATA_SIZE(m_sRenderAPI->m_sSceneRenderData.PointLightCount), m_sSubmitedPointLightRenderData.data(), frameIndex);
...
void shade::Renderer::EndRender(SharedPointer<RenderCommandBuffer>& commandBuffer, SharedPointer<RenderPipeline>& pipeline, std::uint32_t frameIndex)
{
	...
	m_sRenderAPI->EndTimestamp(commandBuffer, pipeline->GetSpecification().Name);
	pipeline->SetTimeQuery(m_sRenderAPI->GetQueryResult(pipeline->GetSpecification().Name));
}
...
void shade::Renderer::SubmitLight(const SharedPointer<GlobalLight>& light, const glm::vec3& direction, const SharedPointer<Camera>& camera)
{
	...
	m_sRenderAPI->m_sSubmitedSceneRenderData.GlobalLightsBuffer->SetData(
		...
		GLOBAL_LIGHT_DATA_SIZE * m_sRenderAPI->m_sSceneRenderData.DirectionalLightCount++);
}
...
void shade::Renderer::SubmitLight(const SharedPointer<PointLight>& light, const glm::mat4& transform, const SharedPointer<Camera>& camera)
{
	...
	assert(RenderAPI::MAX_POINT_LIGHTS_COUNT >= m_sRenderAPI->m_sSceneRenderData.PointLightCount + 1, "Current point light count > RenderAPI::MAX_POINT_LIGHTS_COUNT");

	m_sSubmitedPointLightRenderData.emplace_back(renderData);
	m_sRenderAPI->m_sSceneRenderData.PointLightCount++;
...
void shade::Renderer::QueryResults(std::uint32_t frameIndex)
{
	m_sRenderAPI->QueryResults(frameIndex);
}

float shade::Renderer::GetQueryResult(const std::string& name)
{
	return m_sRenderAPI->GetQueryResult(name);
}
