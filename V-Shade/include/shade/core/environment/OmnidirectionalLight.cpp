...
shade::OmnidirectionalLight::RenderData shade::OmnidirectionalLight::GetRenderData(const glm::vec3& position, const SharedPointer<Camera>& camera) const
{
    ...
    renderData.Cascades[0] = GetLightCascade(fov, position, glm::vec3(1.0, 0.0, 0.0),  glm::vec3(0.0, -1.0, 0.0),  zNear, distance);	// x
    renderData.Cascades[1] = GetLightCascade(fov, position, glm::vec3(-1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0),  zNear, distance);    //-x
    renderData.Cascades[2] = GetLightCascade(fov, position, glm::vec3(0.0, 1.0, 0.0),  glm::vec3(0.0, 0.0, 1.0),   zNear, distance);	// y
    renderData.Cascades[3] = GetLightCascade(fov, position, glm::vec3(0.0, -1.0, 0.0), glm::vec3(0.0, 0.0, -1.0),  zNear, distance);    //-y
    renderData.Cascades[4] = GetLightCascade(fov, position, glm::vec3(0.0, 0.0,  1.0), glm::vec3(0.0, -1.0, 0.0),  zNear, distance);	// z
    renderData.Cascades[5] = GetLightCascade(fov, position, glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.0, -1.0, 0.0),  zNear, distance);	//-z
